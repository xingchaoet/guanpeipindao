...
//select count(*) from ecs_book where isbn = '' and jz= '1' and kc > '0';
        ...
        $sql = "select isbn,amount from bs_yudingdan_mx where sheet_no = '$ydd_pc'";
...
//查找是否已有相同的批次id
        $sql = ser(bs_zhengdingdan, "count(*) as sum,ydd_detail", "zdd_pc_id ='$zdd_pc'");
        $rs = $ms->sdb($sql);
        $data = odbc_fetch_array($rs);
        $sum = $data['sum'];
...
//            删除目录表
            ...
            for ($i = 0; $i < count($transfer_isbn); $i++) {
                ...
                if (!$rs) {
                    echo "从预订单移除isbn号为$transfer_isbn[$i]的书籍成功失败\n";
                } else {
                    echo "从预订单移除isbn号为$transfer_isbn[$i]的书籍成功成功\n";
                }
            }
        }
